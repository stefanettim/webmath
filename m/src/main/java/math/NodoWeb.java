...
public class NodoWeb // extends Canvas
{ // Image offIm;
//    Graphics offGr;
	...
	public void calc_nodo(double fi) // equazione linea
	{
		...
		z = rg * (0.24 + Math.sin(fi + 0 * pi / 3)) * Math.cos(fi + 0 * pi / 3) + rg / 5 * Math.sin(3 * fi + pi / 3);
	}
...
// calcola rete di poligoni,distanza dall' osservatore,inclinazione luce.
	...
	/*
	 * public void paint(Graphics g) { if(offIm!=null) g.drawImage(offIm,0,0,this);}
	 * // fine paint
	 * 
	 * public void update(Graphics g) { if(offIm!=null) disegna_nodo(offGr);
	 * paint(g); }
	 */
